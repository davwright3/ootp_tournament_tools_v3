...
class PitcherSlideFrame(tk.Frame):
    def __init__(self, parent):
        ...
        if self.selected_rank < len(self.slide_df):
            ...
            try:
                self.selected_player_df = (
                    self.slide_df.iloc[[len(self.slide_df) - 1]])
            except Exception:
        ...
        row = 0
        self.title_label = tk.Label(
            self, text="Pitcher Slide", font=fonts.slideshow_header_font)
        self.title_label.grid(column=0, row=row, sticky='nsew', columnspan=5)
        row += 1

        self.bats_label = BatsThrowsLabel(
            self, label_type='Bats',
            side_id=self.selected_player_df.iloc[0]['Bats'])
        self.bats_label.grid(column=0, row=row, sticky='nsew')

        self.overall_rating_label = PlayerOverallRatingLabel(
            self, value=self.selected_player_df.iloc[0]['Val'])
        self.overall_rating_label.grid(column=1, row=row, sticky='nsew')

        self.throws_label = BatsThrowsLabel(
            self, label_type='Throws',
            side_id=self.selected_player_df.iloc[0]['Throws'])
        self.throws_label.grid(column=2, row=row, sticky='nsew')
        row += 1

        self.ratings_frame = PlayerCardPitcherRatingsFrame(
            self, self.selected_player_df)
        self.ratings_frame.grid(row=row, column=0, sticky='nsew')

        self.pitcher_profile_frame = PlayerCardPitcherProfileFrame(
            self, self.selected_player_df)
        self.pitcher_profile_frame.grid(row=row, column=1, sticky='nsew')

        self.individual_pitches_frame = PlayerCardIndividualPitchRatingsFrame(
            self, self.selected_player_df)
        self.individual_pitches_frame.grid(row=row, column=2, sticky='nsew')
        row += 1

        self.stats_frame = PitcherSlideshowStatsFrame(
            self, self.selected_player_df, numquals=len(self.slide_df))
        self.stats_frame.grid(row=row, column=0, sticky='nsew', columnspan=3)
        row += 1

        self.all_pitchers_button = tk.Button(
            self, text='All', command=self.set_all_pitchers)
        self.all_pitchers_button.grid(row=row, column=0, sticky='nsew')

        self.starting_pitchers_button = tk.Button(
            self, text='SP', command=self.set_starting_pitchers)
        self.starting_pitchers_button.grid(row=row, column=1, sticky='nsew')

        self.relief_pitchers_button = tk.Button(
            self, text='RP', command=self.set_relief_pitchers)
        self.relief_pitchers_button.grid(row=row, column=2, sticky='nsew')
        row += 1

        self.previous_button = tk.Button(
            self, text='Previous', command=self.set_previous)
    ...
    def update_slide(self, rank):
        ...
        self.bats_label.update_rating(
            'Bats', self.selected_player_df.iloc[0]['Bats'])
        self.overall_rating_label.update_overall_rating_label(
            self.selected_player_df.iloc[0]['Val'])
        self.throws_label.update_rating(
            'Throws', self.selected_player_df.iloc[0]['Throws'])
        ...
        self.stats_frame.update_frame(
            self.selected_player_df, num_quals=len(self.slide_df))
