...
class ModelParametersFrame(tk.Frame):
    def __init__(self, parent):
        ...
        def set_model_params_frame():
            ...
            if self.selected_model.get() == 'RidgeCV':
                self.alphas_label = tk.Label(
                    self.params_input_frame, text="Alphas")
                self.alphas_label.grid(
                    row=0, column=0, padx=1, pady=1, columnspan=4)

                self.alpha_one_entry = tk.Entry(
                    self.params_input_frame, width=10,
                    textvariable=self.alpha_one)
                self.alpha_one_entry.grid(row=1, column=0, padx=1, pady=1)

                self.alpha_two_entry = tk.Entry(
                    self.params_input_frame, width=10,
                    textvariable=self.alpha_two)
                self.alpha_two_entry.grid(row=1, column=1, padx=1, pady=1)

                self.alpha_three_entry = tk.Entry(
                    self.params_input_frame, width=10,
                    textvariable=self.alpha_three)
                self.alpha_three_entry.grid(row=1, column=2, padx=1, pady=1)

                self.alpha_four_entry = tk.Entry(self.params_input_frame,
                                                 width=10,
                                                 textvariable=self.alpha_four)
                ...
                self.cv_entry = tk.Entry(self.params_input_frame, width=10,
                                         textvariable=self.cv_set)
                self.cv_entry.grid(row=1, column=4, padx=1, pady=1)

                self.test_size_label = tk.Label(self.params_input_frame,
                                                text="Test Size")
                self.test_size_label.grid(row=0, column=5, padx=1, pady=1)

                self.test_size_entry = tk.Entry(self.params_input_frame,
                                                width=10,
    ...
    def get_params(self):
        ...
        elif self.selected_model.get() in (
