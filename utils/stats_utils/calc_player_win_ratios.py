...
def calc_player_win_ratios(
    ...
    req_wins_per_round[num_rounds + 1] = (
            req_wins_per_round[next(reversed(req_wins_per_round))] +
            math.ceil(games_per_finals / 2))
    ...
    team_wins_df = (stats.copy()[['ORG', 'Trny', 'W']]
                    .groupby(['ORG', 'Trny']).sum().reset_index())
    ...
    for rnd in req_wins_per_round.keys():
        result[f'R{rnd}%'] = round(
            (result[f'R{rnd}'] / result['App']) * 100, 1)

    keys = list(total_teams_in_rnd.keys())
    for rnd in keys[1:]:
        result[f'R{rnd}% T'] = (
            round((result[f'R{rnd}'] / total_teams_in_rnd[rnd] * 100), 1))

    result = result.rename(columns={f'R{keys[-1]}': 'CH',
                                    f'R{keys[-2]}': 'F',
                                    f'R{keys[-3]}': 'SF'}
                           )
    result = result.rename(columns={f'R{keys[-1]}%': 'CH%',
    ...
    result = result.rename(columns={f'R{keys[-1]}% T': 'CH% T',
                                    f'R{keys[-2]}% T': 'F% T',
                                    f'R{keys[-3]}% T': 'SF% T'}
                           )
    ...
    if player_search_term is not None:
        cards = cards[cards['Title'].str.contains(
            player_search_term, case=False)]

    final_result = pd.merge(cards, result, on='CID', how='inner')
    final_result = final_result[['Title', 'Val', 'App', 'SF', 'F', 'CH', 'SF%',
                                 'F%', 'CH%', 'SF% T', 'F% T', 'CH% T']]
