...
def fit_model(df, model_name, header_name, player_type):
    ...
    left_model = joblib.load(f'{target_folder}/models'
                             f'/{model_name}_model_left.pkl')
    left_scaler = joblib.load(f'{target_folder}/models'
                              f'/{model_name}_scaler_left.pkl')

    right_model = joblib.load(f'{target_folder}/models'
                              f'/{model_name}_model_right.pkl')
    right_scaler = joblib.load(f'{target_folder}/models'
                               f'/{model_name}_scaler_right.pkl')

    features_file = joblib.load(f'{target_folder}/models'
                                f'/{model_name}_features.pkl')
    ...
    if not valid_left_players.empty:
        ...
        df.loc[valid_left_players, header_name] = (
            left_model.predict(x_new_left_scaled))
    ...
    if not valid_right_players.empty:
        ...
        df.loc[valid_right_players, header_name] = (
            right_model.predict(x_new_right_scaled))
