...
def fit_batters_model(df, model_name, header_name, feature_list):
    ...
    valid_left_walks = df.loc[left_mask, feature_list].dropna().index
    valid_right_walks = df.loc[right_mask, feature_list].dropna().index

    if not valid_left_walks.empty:
        x_new_left = df.loc[valid_left_walks, feature_list]
    ...
    if not valid_right_walks.empty:
        x_new_right = df.loc[valid_right_walks, feature_list]
