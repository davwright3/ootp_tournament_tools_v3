...
from utils.view_utils.player_card_pitching_stats_frame import (
...
class PitcherCard(tk.Toplevel):
    def __init__(self, card_id=None, team_select=None):
        ...
        self.header_frame = Header(
            self,
            app_name=f'Pitcher Card '
                     f'for {player_ratings_df.iloc[0]["//Card Title"]}')
        ...
        self.pitcher_ratings_frame = PlayerCardPitcherRatingsFrame(
            self.main_frame, player_ratings_df)
        ...
        self.individual_pitch_frame = PlayerCardIndividualPitchRatingsFrame(
            self.main_frame, player_ratings_df)
        self.individual_pitch_frame.grid(
            row=1, column=0, sticky='nsew', columnspan=2)

        self.player_stats_frame = PlayerCardPitchingStatsFrame(
            self.main_frame, card_id=card_id, team_select=team_select)
