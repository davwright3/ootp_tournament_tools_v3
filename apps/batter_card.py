...
class BatterCard(tk.Toplevel):
    def __init__(self, card_id=None, selected_team=None):
        ...
        self.header_frame = Header(
            self,
            app_name=f'Batter Card for {player_df.iloc[0]["//Card Title"]}'
        )
        ...
        self.batting_ratings_frame = BatterRatingFrame(
            self.main_frame, df=player_df)
        self.batting_ratings_frame.grid(row=0, column=0, sticky='nsew')

        self.baserunning_frame = BaserunningProfileFrame(
            self.main_frame, df=player_df)
        ...
        self.defense_ratings_frame = PlayerCardDefenseRatingsFrame(
            self.main_frame, df=player_df)
        self.defense_ratings_frame.grid(
            row=1, column=0, columnspan=3, sticky='nsew')

        self.player_batting_stats_frame = PlayerBattingStatsFrame(
            self.main_frame,
            card_id=int(card_id),
