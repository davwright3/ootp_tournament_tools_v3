...
from utils.view_utils.player_overall_rating_label import (
    PlayerOverallRatingLabel)
from utils.view_utils.bats_throws_label import BatsThrowsLabel
from utils.view_utils.player_card_defense_ratings_frame import (
    PlayerCardDefenseRatingsFrame)
from utils.view_utils.batter_profile_frame import BatterProfileFrame
from utils.view_utils.batter_slideshow_stats_frame import (
    BatterSlideshowStatsFrame)
...
class BatterSlideshowApp(tk.Toplevel):
    def __init__(self):
        ...
        self.bat_side_label = BatsThrowsLabel(
            self.main_frame,
            label_type='Bats',
            side_id=self.batter_df.iloc[0]['Bats']
        )
        self.bat_side_label.grid(row=row, column=0, sticky="nsew")

        self.player_value_label = PlayerOverallRatingLabel(
            self.main_frame, self.batter_df.iloc[0]['Val'])
        self.player_value_label.grid(row=row, column=1, sticky="nsew")

        self.throws_label = BatsThrowsLabel(
            self.main_frame,
            'Throws',
            self.batter_df.iloc[0]['Throws']
        )
        self.throws_label.grid(row=row, column=2, sticky="nsew")
        row += 1

        self.batting_ratings_frame = BatterRatingFrame(
            self.main_frame, self.batter_df)
        self.batting_ratings_frame.grid(row=row, column=0, sticky="nsew")

        self.defense_positions_frame = PlayerCardDefenseRatingsFrame(
            self.main_frame, self.batter_df)
        self.defense_positions_frame.grid(row=row, column=1, sticky="nsew")

        self.batter_profile_frame = BatterProfileFrame(
            self.main_frame, self.batter_df)
        self.batter_profile_frame.grid(row=row, column=2, sticky="nsew")

        self.baserunning_profile_frame = BaserunningProfileFrame(
            self.main_frame, self.batter_df)
        self.baserunning_profile_frame.grid(row=row, column=3, sticky="nsew")
        row += 1

        self.batter_stats_frame = BatterSlideshowStatsFrame(
            self.main_frame, self.batter_df)
        self.batter_stats_frame.grid(
            row=row, column=0, sticky="nsew", columnspan=4)
        row += 1
        ...
        self.next_button = tk.Button(
            self.main_frame, text="NEXT", command=self.next_batter)
        self.next_button.grid(row=row, column=2, sticky="nsew")
        row += 1

        self.catcher_button = tk.Button(
            self.main_frame,
            text="CATCH",
            command=lambda: self.change_position(position='LearnC')
        )
        self.catcher_button.grid(row=row, column=0, sticky="nsew")

        self.first_base_button = tk.Button(
            self.main_frame,
            text='1B',
            command=lambda: self.change_position(position='Learn1B')
        )
        self.first_base_button.grid(row=row, column=1, sticky="nsew")

        self.second_base_button = tk.Button(
            self.main_frame,
            text='2B',
            command=lambda: self.change_position(position='Learn2B')
        )
        self.second_base_button.grid(row=row, column=2, sticky="nsew")
        row += 1

        self.third_base_button = tk.Button(
            self.main_frame,
            text='3B',
            command=lambda: self.change_position(position='Learn3B')
        )
        self.third_base_button.grid(row=row, column=0, sticky="nsew")

        self.shortstop_button = tk.Button(
            self.main_frame,
            text='SS',
            command=lambda: self.change_position(position='LearnSS')
        )
        self.shortstop_button.grid(row=row, column=1, sticky="nsew")

        self.left_field_button = tk.Button(
            self.main_frame, text='LF',
            command=lambda: self.change_position(position='LearnLF')
        )
        self.left_field_button.grid(row=row, column=2, sticky="nsew")
        row += 1

        self.center_field_button = tk.Button(
            self.main_frame,
            text='CF',
            command=lambda: self.change_position(position='LearnCF')
        )
        self.center_field_button.grid(row=row, column=0, sticky="nsew")

        self.right_field_button = tk.Button(
            self.main_frame,
            text='RF',
            command=lambda: self.change_position(position='LearnRF')
        )
        self.right_field_button.grid(row=row, column=1, sticky="nsew")

        self.all_batters_button = tk.Button(
            self.main_frame,
            text='ALL',
            command=lambda: self.change_position(position=None)
        )
    ...
    def update_batter(self, rank):
        if not self.updating:
            ...
            self.player_title.configure(
                text=f"{self.rank_var.get()}: "
                     f"{self.batter_df.iloc[0]['Title']}")
            self.player_value_label.update_overall_rating_label(
                self.batter_df.iloc[0]['Val'])
            self.bat_side_label.update_rating(
                'Bats', self.batter_df.iloc[0]['Bats'])
            self.throws_label.update_rating(
                'Throws', self.batter_df.iloc[0]['Throws'])
            ...
            self.batter_stats_frame.update_batter(
                self.batter_df, quals=num_qualifiers)
            self.updating = False
